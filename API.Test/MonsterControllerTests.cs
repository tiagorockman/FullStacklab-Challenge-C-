...
public class MonsterControllerTests
{
    ...
    {
        string filePath = Path.Combine($@"{Directory.GetParent(Directory.GetCurrentDirectory())!.Parent.Parent}\Files\monsters-correct.csv");
        var csvFileData = File.OpenRead(filePath);
        var formFile = new FormFile(csvFileData, 0, csvFileData.Length, name: "stream", fileName: "monsters-correct.csv");

     
        var mockMonster = It.IsAny<IEnumerable<Monster>>();
        this._repository
            .Setup(x => x.Monsters.AddAsync(mockMonster)).Verifiable();
        this._repository
            .Setup(x => x.Save()).Verifiable();

        MonsterController monsterController = new MonsterController(_repository.Object);
        ActionResult result = await monsterController.ImportCsv(formFile);
